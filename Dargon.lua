...
Autofarm:addToggle('🛡 Endurance Animation', function(bool)
    getgenv().aend = bool -- :addToggle(Title, callback)
    if bool then
    anend()
        end
    end)    
...
Autofarm:addToggle('Fire Golem', function(bool)
getgenv().Fire Golem = bool -- :addToggle(Title, callback)
if bool then
Fire Golem()
    end
end)
...
function anend()
        while getgenv().aend == true do
local args = {[1] = "Animation",[2] = "Play",[3] = "Endurance"}
game:GetService("ReplicatedStorage").Server:FireServer(unpack(args))
            wait(1.2)
        end
end
